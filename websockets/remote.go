...
// Synchronously submit a raw transaction
func (r *Remote) SubmitRaw(raw []byte) (*SubmitResult, error) {
	cmd := &SubmitCommand{
		Command: newCommand("submit"),
		TxBlob:  fmt.Sprintf("%X", raw),
	}
...
func (r *Remote) Subscribe(accounts []string, ledger, transactions, transactionsProposed, server bool) (*SubscribeResult, error) {
	streams := []string{}
	if ledger {
